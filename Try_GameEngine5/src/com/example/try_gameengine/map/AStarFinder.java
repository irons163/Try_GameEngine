...
public class AStarFinder implements Runnable {
	...
	private Set<Vector2f> visitedCache;

	private ScoredPath spath;

	private static Vector2f start, over;
	...
	public static LinkedList<Vector2f> find(int[][] maps, int x1, int y1,
	...
	public static LinkedList<Vector2f> find(Field2D maps, int x1, int y1,
			int x2, int y2, boolean flag) {
		if (astar == null) {
	...
	public static LinkedList<Vector2f> find(Field2D maps, Vector2f start,
	...
	public static LinkedList<Vector2f> find(int[][] maps, Vector2f start,
			Vector2f goal, boolean flag) {
		if (astar == null) {
			astar = new AStarFinder();
		}
	...
	private void insert(int score, LinkedList<Vector2f> path) {
	...
	private class ScoredPath {
		...
		ScoredPath(int score, LinkedList<Vector2f> path) {
			this.score = score;
			this.path = path;
		}
