...
public class TileMap {
	...
	private Vector2f scrollingVelocity;
	...
	public Bitmap getTileFromPixels(float x, float y) {
		return getTileFromPixels(new Vector2f(x, y));
	}

	public Bitmap getTileFromPixels(Vector2f p) {
	...
	public Vector2f getCollision(Vector2f p, float width, float height) {
		Bitmap tile1 = getTileFromPixels(p.getX(), p.getY());
		Bitmap tile2 = getTileFromPixels(p.getX(), p.getY() + height);
		Bitmap tile3 = getTileFromPixels(p.getX() + width, p.getY());
		float x, y;
		x = y = 0;
		if (tile1 != null) {
			x = -1;
	 ...
	 * @param velocity
	 */
	public void setScrollingVelocity(Vector2f velocity) {
	 ...
	 * @return
	 */
	public Vector2f getScrollingVelocity() {
		return scrollingVelocity;
	}
