...
public class FontObject {
	...
	public static FontObject getDefaultFont() {
		return FontObject.getFont(LSystem.FONT_NAME, 0, 200);
	}

	public static FontObject getFont(int size) {
		return FontObject.getFont(LSystem.FONT_NAME, 0, size);
	}

	public static FontObject getFont(String familyName, int size) {
		return getFont(familyName, 0, size);
	}

	public static FontObject getFont(String familyName, int style, int size) {
		...
		if (o == null) {
			...
			fonts.put(name, o = new FontObject(paint, size));
		}
		return (FontObject) o;
	}

	public static FontObject getFromAssetFont(String path, int style, int fontSize) {
		return new FontObject(Typeface.DEFAULT, path, fontSize);
	}

	public static FontObject getFont(int face, int style, int fontSize) {
		FontObject font = new FontObject(fontSize);
		return getFont(font, face, style, fontSize);
	}

	public static FontObject getFont(FontObject font, int face, int style, int fontSize) {
		int paintFlags = 0;
	...
	private FontObject(int fontSize) {
		this.fontSize = fontSize;
	}

	private FontObject(Typeface typeface, int fontSize) {
		this(createPaint(typeface), fontSize);
	}

	private FontObject(Paint typefacePaint, int fontSize) {
		this.fontSize = fontSize;
		this.setTypefacePaint(typefacePaint);
	}

	private FontObject(Typeface typeface, String path, int fontSize) {
		this(createPaint(typeface), path, fontSize);
	}

	private FontObject(Paint typefacePaint, String path, int fontSize) {
		Typeface face = Typeface.createFromAsset(LSystem.getSystemHandler()
				.getContext().getAssets(), path);
		this.fontSize = fontSize;
	...
	public float getScale() {
		...
		float scale;
		if (fontSize == FontObject.SIZE_LARGE) {
			scale = 1.5F;
		} else if (fontSize == FontObject.SIZE_SMALL) {
			scale = 0.8F;
		} else {
			scale = 1;
		}
