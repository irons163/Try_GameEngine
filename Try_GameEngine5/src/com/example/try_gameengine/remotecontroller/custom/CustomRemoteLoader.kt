...
class CustomRemoteLoader {
    private val upKey: Key? = null
    private val downKey: Key? = null
    private val rightKey: Key? = null
    private val leftKey: Key? = null
    private val enterKey: Key? = null
    private val cancelKey: Key? = null
    private var remoteControl: CustomRemoteControl? = null
    var keys: MutableList<Key?> = ArrayList<Key?>()
    ...
    private fun load() {
        remoteControl = CustomRemoteControl()
		...
		// */
        for (key in keys) {
            val pressDown = CustomCommandPressDown(key!!)
            val pressUp = CustomCommandPressDown(key)
            remoteControl!!.addCommand(pressDown, pressUp)
        }
    }

    fun getRemoteControl(): CustomRemoteControl {
