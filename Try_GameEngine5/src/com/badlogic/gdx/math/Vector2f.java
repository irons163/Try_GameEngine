...
public class Vector2f implements Serializable {
	...
	public int angle(Vector2f v) {
		...
		float rwinkel = (float) Math.atan(ady / adx);
		float dwinkel = 0.0f;
		if ((dx > 0) && (dy > 0)) {
			dwinkel = (float) Math.toDegrees(rwinkel);
		} else if ((dx < 0) && (dy > 0)) {
			dwinkel = (float) (180.0f - Math.toDegrees(rwinkel));
		} else if ((dx > 0) && (dy < 0)) {
			dwinkel = (float) (360.0f - Math.toDegrees(rwinkel));
		} else if ((dx < 0) && (dy < 0)) {
			dwinkel = (float) (180.0f + Math.toDegrees(rwinkel));
		}
	...
	public void setAngle(float angle) {
		...
		float len = length();
		x = len * (float) Math.cos(StrictMath.toRadians(angle));
		y = len * (float) Math.sin(StrictMath.toRadians(angle));
	...
	public static Vector2f getRealCordinates(Vector2f relativeCoordinate,
			Vector2f centroid, float angularPosition) {
		Vector2f answer = (new Matrix2f(angularPosition)).multiply(
				relativeCoordinate).add(centroid);
		return answer;
